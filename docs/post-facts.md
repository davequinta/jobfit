...
| 334 | HN drop rate "from 7.8% to 5.3%, and what remains is mostly `[flagged]`" | 7.8% = run 3 (2026-08-22), 19 of 243. No stored run gives 5.3%. Run 5 (2026-09-08): 8 of 242, 3.3% — 200 new postings, 34 already stored and seen again, 8 dropped. None of run 5's 8 dropped comments is `[flagged]`; 2 of run 3's 19 are | `SELECT run_id, kind, count(*) FROM ingest_issues WHERE source = 'hackernews' GROUP BY 1, 2`; `SELECT count(*) FROM postings WHERE source = 'hackernews' AND first_seen_run = 5` → 200; `… AND first_seen_run < 5 AND last_seen_at >= (SELECT started_at FROM ingest_runs WHERE id = 5)` → 34; `SELECT run_id, sample FROM ingest_issues WHERE source = 'hackernews'` |
...
| HN drop rate 5.3% | The re-parse that produced it was not stored |
